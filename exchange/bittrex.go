...
func (b bittrex) marketToSymbol(market MarketID) string {
	return market.Quote() + "-" + market.Base()
}

func (b bittrex) GetOpen(id MarketID) (float64, error) {
	respJSON := latestTickResponse{}
	err := httpGetJSON("https://bittrex.com/Api/v2.0/pub/market/GetLatestTick?marketName="+b.marketToSymbol(id)+"&tickInterval=day", &respJSON)
	if err != nil {
		return 0, err
	}
	...
	openPrice := respJSON.Result[0].Open
	return openPrice, nil
}

func (b bittrex) GetLast(id MarketID) (float64, error) {
	respJSON := gettickerResponse{}
	err := httpGetJSON("https://bittrex.com/api/v1.1/public/getticker?market="+b.marketToSymbol(id), &respJSON)
	if err != nil {
		return 0, err
	}
	...
	lastPrice := respJSON.Result.Last
	return lastPrice, nil
}

func (b *bittrex) Listen(ctx context.Context, update chan<- Market) error {
	...
	msgHandler := func(msg signalr.Message) {
		if len(msg.M) > 0 && msg.M[0].M == "uL" {
			...
			for idx := range b.markets {
				liteResp, err := resp.Get(b.marketToSymbol(b.markets[idx]))
				if err != nil {
					continue
				}
				b.markets[idx].LastPrice = liteResp.Last
